...
```{r}
get_rxcui <- drugid_process %>% filter(drug_id_type== "rxcui")
...
split_rxcui <- split_dataframe(get_rxcui, 10)

# Use mclapply to apply the function in parallel
rxcui_results <- list()

# trial_rxcui_results <- list()
# 
# for (i in seq_len(min(3, length(split_rxcui)))) {
#   cat("Processing split_rxcui index:", i, "\n")
#   cat("Structure of split_rxcui[[", i, "]]:\n")
#   str(split_rxcui[[i]])
# 
#   # Call the function
#   split_rxcui_df <- as.data.frame(split_rxcui[[i]])
#   result <- map_newdrug_id(split_rxcui_df, "rxcui", "47")
# 
#   # Check the result and add to the list
#   if (!is.null(result)) {
#     trial_rxcui_results[[length(trial_rxcui_results) + 1]] <- result
#   } else {
#     cat("Result is NULL for index:", i, "\n")
#   }
# }
# 
# # Combine results
# trial_final_result <- do.call(rbind, trial_rxcui_results)
# print(trial_final_result)


# Process each smaller dataframe using a loop
for (i in seq_along(split_rxcui)) {
  # Apply the function to the current dataframe
  rxcui_results[[i]] <- map_newdrug_id(split_rxcui[[i]],"rxcui", "47")
}

# Combine the results if needed (optional, depends on function output)
final_result <- do.call(rbind, rxcui_results)
