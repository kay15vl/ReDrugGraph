...
df <- drugid_process
drug_id_type <- "drugbank"
source_num <- "2"
filename <- "rugbank_mapping.RData"
map_chembl <- function(df, drug_id_type, source_num, filename){
  druginfo_df <- drugid_process %>% filter(drug_id_type == drug_id_type)
...
```{r}
drugbank_df <- map_chembl(drugid_process, "drugbank", "2", "drugbank_mapping.RData")
```
...
### Process iuphar
```{r}
iuphar_df <- drugid_process %>% filter(drug_id_type == 'iuphar.ligand')
iuphar_ids <- as.list(iuphar_df$drug_num)
iuphar_to_chembl <- process_drug_ids(iuphar_ids, "4")
iuphar_to_chembl
save(drugdb_to_chembl, file ="../data/raw/iuphar_mapping.RData") 
```
